...
export async function setOpenRouterApiKey(context: vscode.ExtensionContext) {
  ...
  if (!input.trim()) {
    await vscode.window.showWarningMessage('Markdown Translator: API key was empty, so the operation was canceled.');
    return;
  }

  const origin = await storeOpenRouterApiKeyForCurrentEndpoint(context, input.trim());
  await vscode.window.showInformationMessage(`Markdown Translator: Saved API key for ${origin} in SecretStorage.`);
}
...
export async function resetOpenRouterApiKey(context: vscode.ExtensionContext) {
  ...
  const hasExisting = await hasOpenRouterApiKeyForCurrentEndpoint(context);
  if (!hasExisting) {
    await vscode.window.showInformationMessage(`Markdown Translator: No OpenRouter API key is saved for ${endpoint.origin}.`);
    return;
  }

  const confirm = await vscode.window.showWarningMessage(
    `Markdown Translator: Delete the saved OpenRouter API key for ${endpoint.origin} from SecretStorage?`,
    { modal: true },
    'Reset',
  );
  ...
  await vscode.window.showInformationMessage(`Markdown Translator: Deleted the OpenRouter API key for ${origin}.`);
}
